...
function SpecialtyChoiceMessage({ 
  ...
  onComplete // 진료과 선택 완료 후 실핼할 콜백 함수 (ex. 메시지 추가용)
}) {
  const handleClick = (specialty) => {
    // 선택된 진료과 처리
    onSelect?.(specialty); // 사용자가 진료과를 클릭했을 때 실행
    onComplete?.(specialty); // 선택 완료 실행되는 후처리 (ex. GPT 응답 메시지 추가)
  }
      ...
      <div className="message-content p-2">
        ...
        <div className="d-flex flex-column align-items-end gap-2">
          ...
          {specialties.map((specialty, idx) => (
            <button
              key={idx} // 고유 키
              className="btn btn-outline-primary btn-sm text-nowrap" // 버튼 스타이 
